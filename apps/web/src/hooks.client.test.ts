...
describe("handleError (client)", () => {
  ...
  it("returns error message from Error instance", () => {
    ...
    const result = handleError({ error: new Error("client error"), event: {} as never, status: 500, message: "" }) as { message: string };
    expect(result.message).toBe("client error");
  });

  it("returns stringified message for non-Error string", () => {
    // eslint-disable-next-line @typescript-eslint/no-unsafe-type-assertion
    const result = handleError({ error: "raw string", event: {} as never, status: 500, message: "" }) as { message: string };
    expect(result.message).toBe("raw string");
  });

  it("returns fallback message when error has no message", () => {
    // eslint-disable-next-line @typescript-eslint/no-unsafe-type-assertion
    const result = handleError({ error: new Error(""), event: {} as never, status: 500, message: "" }) as { message: string };
  ...
  it("describes SvelteKit Redirect objects with status and location", () => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-unsafe-type-assertion
    const result = handleError({ error: redirectObj, event: {} as never, status: 500, message: "" }) as { message: string };
  ...
  it("describes SvelteKit HttpError objects with status and body", () => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-unsafe-type-assertion
    const result = handleError({ error: httpError, event: {} as never, status: 500, message: "" }) as { message: string };
  ...
  it("JSON-serializes unknown plain objects instead of [object Object]", () => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-unsafe-type-assertion
    const result = handleError({ error: plainObj, event: {} as never, status: 500, message: "" }) as { message: string };
  ...
  it("falls back to String() for objects that cannot be JSON-serialized", () => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-unsafe-type-assertion
    const result = handleError({ error: circular, event: {} as never, status: 500, message: "" }) as { message: string };
