<script lang="ts">
  ...
  const lead = $derived(data.lead as { id: string; displayId: string; firstName: string; lastName: string });
  ...
  const leadName = $derived(`${lead.firstName} ${lead.lastName}`);
...
<RecordManagementBar
  backHref={`/leads/general-leads/${lead.id}`}
  backLabel="General Lead"
  title={`${lead.displayId} — ${leadName}`}
>
  {#snippet actions()}
    <div class="relative">
      <Search
