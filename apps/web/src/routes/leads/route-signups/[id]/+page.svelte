<script lang="ts">
  ...
  async function linkEmail(emailId: string) {
    linking = true;
    try {
      await api(`/api/emails/${emailId}`, { method: "PATCH", body: JSON.stringify({ routeSignupId: signup.id }) });
  ...
  async function linkPhone(phoneId: string) {
    linking = true;
    try {
      await api(`/api/phone-numbers/${phoneId}`, { method: "PATCH", body: JSON.stringify({ routeSignupId: signup.id }) });
  ...
  async function linkSocialId(socialIdId: string) {
    linking = true;
    try {
      await api(`/api/social-ids/${socialIdId}`, { method: "PATCH", body: JSON.stringify({ routeSignupId: signup.id }) });
    ...
    >
      ...
      {#snippet addForm()}
        ...
        {:else}
          <div class="space-y-3">
            <div>
              <label for="rouLinkEmail" class="block text-sm font-medium text-text-secondary">Search emails</label>
              <input id="rouLinkEmail" type="text" bind:value={emailLinkQuery} oninput={searchExistingEmails} class="glass-input mt-1 block w-full px-3 py-2 text-sm" placeholder="Type to search existing emails..." />
            </div>
    ...
    >
      ...
      {#snippet addForm()}
        ...
        {:else}
          <div class="space-y-3">
            <div>
              <label for="rouLinkPhone" class="block text-sm font-medium text-text-secondary">Search phone numbers</label>
              <input id="rouLinkPhone" type="text" bind:value={phoneLinkQuery} oninput={searchExistingPhones} class="glass-input mt-1 block w-full px-3 py-2 text-sm" placeholder="Type to search existing phone numbers..." />
            </div>
            {#if linkSearching}<p class="text-xs text-text-muted">Searching...</p>{/if}
    ...
    >
      ...
      {#snippet addForm()}
        ...
        {:else}
          <div class="space-y-3">
            <div>
              <label for="rouLinkSocialId" class="block text-sm font-medium text-text-secondary">Search social IDs</label>
              <input id="rouLinkSocialId" type="text" bind:value={socialIdLinkQuery} oninput={searchExistingSocialIds} class="glass-input mt-1 block w-full px-3 py-2 text-sm" placeholder="Type to search existing social IDs..." />
            </div>
            {#if linkSearching}<p class="text-xs text-text-muted">Searching...</p>{/if}
