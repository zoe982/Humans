<script lang="ts">
  ...
  import { evacuationLeadStatusLabels, ACTIVITY_TYPE_OPTIONS } from "$lib/constants/labels";
  ...
  const lead = $derived(data.evacuationLead as EvacuationLead);
  ...
  $effect(() => {
    if (leadScore == null) {
      api("/api/lead-scores/ensure", {
        method: "POST",
        body: JSON.stringify({ parentType: "evacuation_lead", parentId: lead.id }),
      }).then((result) => {
  ...
  async function linkEmail(emailId: string) {
    linking = true;
    try {
      await api(`/api/emails/${emailId}`, { method: "PATCH", body: JSON.stringify({ evacuationLeadId: lead.id }) });
  ...
  async function linkPhone(phoneId: string) {
    linking = true;
    try {
      await api(`/api/phone-numbers/${phoneId}`, { method: "PATCH", body: JSON.stringify({ evacuationLeadId: lead.id }) });
  ...
  async function linkSocialId(socialIdId: string) {
    linking = true;
    try {
      await api(`/api/social-ids/${socialIdId}`, { method: "PATCH", body: JSON.stringify({ evacuationLeadId: lead.id }) });
  ...
  const createNewHumanUrl = $derived.by(() => {
    ...
    params.set("fromEvacuationLead", lead.id);
    if (lead.first_name) params.set("firstName", lead.first_name);
    if (lead.middle_name) params.set("middleName", lead.middle_name);
  ...
  async function handleStatusChange(newStatus: string) {
    ...
    try {
      ...
      getStore("evacuation-leads").patchItem(lead.id, { status: newStatus });
      await invalidateAll();
    } catch {
  ...
  const interestsList = $derived(formatInterests(lead.interests));
</script>

<svelte:head>
  <title>{lead.display_id ? lead.display_id + ' — ' : ''}{displayName(lead)} - Evacuation Lead - Humans</title>
</svelte:head>

<div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
  <!-- Record Management Bar -->
  <RecordManagementBar
    backHref="/leads/evacuation-leads"
  ...
  {#if form?.error}
    {#if form.code?.endsWith("_DUPLICATE") && form.details}
      <DuplicateContactBanner
        ...
        parentType="evacuationLead"
        parentId={lead.id}
        parentField="evacuationLeadId"
      />
    {:else}
  ...
  {#if !isClosed}
    <div class="mb-6">
      <NextActionSection
        apiEndpoint={`/api/evacuation-leads/${lead.id}/next-action`}
        {colleagueOptions}
        {currentColleagueId}
        ...
        <div>
          ...
          <select id="sourceSelect" name="source" class="glass-input block w-full px-3 py-2 text-sm">
            ...
            {#each leadSources as src, i (i)}
              <option value={src.name} selected={lead.crm_source === src.name}>{src.name}</option>
            {/each}
          </select>
        </div>
        <div>
          ...
          <select id="channelSelect" name="channel" class="glass-input block w-full px-3 py-2 text-sm">
            ...
            {#each leadChannels as ch, i (i)}
              <option value={ch.name} selected={lead.crm_channel === ch.name}>{ch.name}</option>
            {/each}
          </select>
        </div>
      </div>
  ...
  <div class="glass-card p-6 mb-6">
    ...
    <dl class="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
      ...
      <div class="sm:col-span-2">
        <dt class="text-sm font-medium text-text-muted">Interests</dt>
        <dd class="mt-1">
          {#if interestsList.length > 0}
            <div class="flex flex-wrap gap-1">
              {#each interestsList as interest, i (i)}
                <span class="glass-badge badge-blue text-xs">{interest}</span>
              {/each}
            </div>
          {:else}
      ...
      <div>
        <dt class="text-sm font-medium text-text-muted">Consent Contact</dt>
        <dd class="mt-1 text-sm text-text-primary">{lead.consent_contact ? "Yes" : "No"}</dd>
      </div>
      <div>
        <dt class="text-sm font-medium text-text-muted">Newsletter Consent</dt>
        <dd class="mt-1 text-sm text-text-primary">{lead.consent_newsletter ? "Yes" : "No"}</dd>
      </div>
      <div>
        <dt class="text-sm font-medium text-text-muted">Created</dt>
        <dd class="mt-1 text-sm text-text-primary">{formatDateTime(lead.inserted_at)}</dd>
      </div>
      <div>
    ...
    >
      ...
      {#snippet addForm()}
        ...
        {:else}
          <div class="space-y-3">
            <div>
              <label for="evaLinkEmail" class="block text-sm font-medium text-text-secondary">Search emails</label>
              <input id="evaLinkEmail" type="text" bind:value={emailLinkQuery} oninput={searchExistingEmails} class="glass-input mt-1 block w-full px-3 py-2 text-sm" placeholder="Type to search existing emails..." />
            </div>
    ...
    >
      ...
      {#snippet addForm()}
        ...
        {:else}
          <div class="space-y-3">
            <div>
              <label for="evaLinkPhone" class="block text-sm font-medium text-text-secondary">Search phone numbers</label>
              <input id="evaLinkPhone" type="text" bind:value={phoneLinkQuery} oninput={searchExistingPhones} class="glass-input mt-1 block w-full px-3 py-2 text-sm" placeholder="Type to search existing phone numbers..." />
            </div>
            {#if linkSearching}<p class="text-xs text-text-muted">Searching...</p>{/if}
    ...
    >
      ...
      {#snippet addForm()}
        ...
        {:else}
          <div class="space-y-3">
            <div>
              <label for="evaLinkSocialId" class="block text-sm font-medium text-text-secondary">Search social IDs</label>
              <input id="evaLinkSocialId" type="text" bind:value={socialIdLinkQuery} oninput={searchExistingSocialIds} class="glass-input mt-1 block w-full px-3 py-2 text-sm" placeholder="Type to search existing social IDs..." />
            </div>
            {#if linkSearching}<p class="text-xs text-text-muted">Searching...</p>{/if}
...
{#if showLossDialog}
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
    <div class="glass-card p-6 max-w-md w-full mx-4">
      ...
      <p class="mt-2 text-sm text-text-secondary">Optionally provide details for closing this lead.</p>
      <div class="mt-3">
        <label for="evaLossReasonSelect" class="block text-sm font-medium text-text-secondary mb-1">Loss Reason</label>
        <select id="evaLossReasonSelect" class="glass-input block w-full px-3 py-2 text-sm" bind:value={selectedLossReason}>
