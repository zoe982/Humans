...
<svelte:head>
  <title>New Referral Code - Humans</title>
</svelte:head>

<div class="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
  <PageHeader
    title="New Referral Code"
    breadcrumbs={[{ label: "Reports", href: "/reports" }, { label: "Referral Codes", href: "/referral-codes" }, { label: "New" }]}
  />
  ...
  <form method="POST" action="?/create" class="space-y-6 glass-card p-6">
    <div>
      <label for="code" class="block text-sm font-medium text-text-secondary mb-1">Code <span class="text-required">*</span></label>
      <input
    ...
    <div class="flex gap-3">
      ...
      <a href={resolve('/referral-codes')} class="btn-ghost">Cancel</a>
    </div>
  </form>
</div>
