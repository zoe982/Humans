...
export async function runScheduledFrontSync(env: Env) {
  ...
  try {
    ...
    console.log(
      `[cron] Front sync complete: imported=${result.imported} skipped=${result.skipped} unmatched=${result.unmatched} errors=${result.errors.length}`,
    );
  } catch (err) {
