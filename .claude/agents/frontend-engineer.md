...
> **CRITICAL: You NEVER run tests.** All test execution happens in the main Bash context (the orchestrator that launched you). After writing code and tests, report the exact test command (with `| tail -n N`) for the orchestrator to run. You write code and tests — the orchestrator verifies them.
...
4. **Report the test command for the orchestrator to run (RED).** Report: `cd <pkg> && pnpm test run <your-test-file> 2>&1 | tail -n 20`. The orchestrator runs it and confirms the red.
...
6. **Report the test command for the orchestrator to run (GREEN).** Report: `cd <pkg> && pnpm test run <your-test-file> 2>&1 | tail -n 20` — the orchestrator confirms it passes and no other tests broke.
...
5. **Report the full suite command.** Not just the new tests. Report: `cd <pkg> && pnpm test run 2>&1 | tail -n 40` for the orchestrator to run. No regressions allowed.
...
10. **Report coverage command.** Report: `cd <pkg> && pnpm test run --coverage 2>&1 | tail -n 80` for the orchestrator to run and confirm thresholds are maintained.
